...
for episode in range(1, num_episodes + 1):
    ...
    while not done:
        ...
        if 0 <= next_state[0] < GRID_SIZE and 0 <= next_state[1] < GRID_SIZE:
            q_table[current_state[0], current_state[1], action_idx] = q_table[current_state[0], current_state[1], action_idx] + alpha * \
        ...
        for i in range(GRID_SIZE):
            for j in range(GRID_SIZE):
                ...
                pygame.draw.rect(win, BLACK, (i * CELL_SIZE,
                                 j * CELL_SIZE, CELL_SIZE, CELL_SIZE), 1)

        for pos in snake.positions:
            pygame.draw.rect(
                win, GREEN, (pos[0] * CELL_SIZE, pos[1] * CELL_SIZE, CELL_SIZE, CELL_SIZE))
            label = font.render("Snake", True, BLACK)
            win.blit(label, (pos[0] * CELL_SIZE + 5, pos[1] * CELL_SIZE + 5))

        pygame.draw.rect(
            win, RED, (food.position[0] * CELL_SIZE, food.position[1] * CELL_SIZE, CELL_SIZE, CELL_SIZE))
        label = font.render("Food", True, BLACK)
        win.blit(
            label, (food.position[0] * CELL_SIZE + 5, food.position[1] * CELL_SIZE + 5))
